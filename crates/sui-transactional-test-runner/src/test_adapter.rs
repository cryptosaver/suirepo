    ...
    fn handle_subcommand(
        ...
        match command {
            ...
            SuiSubcommand::UpgradePackage(UpgradePackageCommand {
                ...
                let state = self.compiled_state();
                let package_id = state.resolve_named_address(package.as_str()).into();
                // Reset package address to 0x0 which resets module addressses to 0x0.
                *state.named_address_mapping.get_mut(&package).unwrap() =
...
impl<'a> SuiTestAdapter<'a> {
    fn upgrade_package(
        ...
        let digest: Vec<u8> =
            MovePackage::compute_digest_for_modules_and_deps(&modules, &dependencies).into();
        ...
        let upgrade_receipt = builder.upgrade(package_id, upgrade_ticket, dependencies, modules);
